...
//    UInt32GetLE
//    *(uint32_t *)(person +  8) = UInt32GetLE(&n);//htole32 host to little endian
//    *(uint32_t *)(person + 12) = UInt32GetLE(&k);//htole32
    UInt32SetLE((uint32_t *)(person +  8), n);
    UInt32SetLE((uint32_t *)(person +  12), k);
}
...
{
//    uint32_t result = 0;
    return UInt32GetBE(array);
//    UInt32SetBE(&array, result);
//    return result;
//    return be32toh(*(uint32_t *)array); //big endian to host
}

static void generateHash(blake2b_state *S, const uint32_t g, uint8_t *hash, const size_t hashLen)
{
//    const uint32_t le_g = UInt32GetLE(&g);//htole32
    uint32_t le_g =0;
    UInt32SetLE(&le_g, g);//htole32
...
//    dump_hex(validBlockData, 140);
    ...
    for (int r = 1; r < k && x_size > 0; r++) {
        ...
        qsort_r(x, x_size, sizeof(hash), (void *)&collisionByteLength, (int (* _Nonnull )(void *, const void *, const void *))compareSR);
//        qsort_r(x, x_size, sizeof(hash), compareSR, (int *)&hashLen);
//        qsort(x, x_size, sizeof(hash), compareSR);
...
bool basicValidator(void *data, const unsigned char *soln)
{
    ...
    for (int j = 0; j < solnr; j++) {
        ...
        int i = 0 ;
//        i = UInt32GetBE(&indices[j]);
        UInt32SetBE(&i,indices[j]); //be32toh
        D(" %d", i);
                   ...
                   int n, int k)
{
    ...
    printf("\nnonce:");
    dump_hex(input+108, 32);
    printf("\n\n");
    blake2b_update(digest, input+108, 32);
    if (!validBlock) {
...
static void hashNonce(blake2b_state *S, uint32_t nonce)
{
    for (int i = 0; i < 8; i++) {
        uint32_t le = 0;
        if (i == 0) {
            UInt32SetLE(&le, nonce);
//            UInt32GetLE(&nonce);//htole32
        }
...
void BREquihash(uint8_t* header, UInt256 nonce) {
    ...
    uint32_t nn = 0;
    dump_hex(header, 108);
    int ret = SolverFunction(header, NULL, NULL, NULL, NULL, 1, n, k);

//    blake2b_state digest[1];
//    struct validData valData = { .n = n, .k = k, .digest = digest };
//    digestInit(digest, n, k);
//    blake2b_update(digest, header, 140);
//    if (basicValidator(&valData, header+140)) {
//        printf("\n\n****VALID\n\n");
//    }else {
//        printf("\n\n****NOOOT VALID\n\n");
//    }
...
//    struct validData valData = { .n = n, .k = k, .digest = digest };
//    digestInit(digest, n, k);
//    blake2b_update(digest, (uint8_t *)header, 140);//strlen(header)
////    hashNonce(digest, nn);
////    blake2b_update(digest, (uint8_t *)&nonce, 32);
//    basicSolve(digest, n, k, basicValidator, &valData);
}

void BREquihashValidate(uint8_t* header, UInt256 nonce) {
    int n = 200;
    int k = 9;
    uint32_t nn = 0;
    dump_hex(header, 108);
